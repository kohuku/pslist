 ...
 */
doca_error_t
pslist(const char *dma_device_name, const char *pci_addr, enum doca_apsh_system_os os_type)
{
	doca_error_t result;
	int num_processes, i;
	...
	result = init_doca_apsh_system(apsh_ctx, os_type, os_symbols, mem_region, pci_addr, &sys);
	if (result != DOCA_SUCCESS) {
