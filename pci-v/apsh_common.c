...
init_doca_apsh_system(struct doca_apsh_ctx *ctx, enum doca_apsh_system_os os_type, const char *os_symbols,
		      const char *mem_region, const char *pci_addr, struct doca_apsh_system **system)
{
	doca_error_t result;
	...
	result = open_doca_device_rep_with_pci(dma_device, DOCA_DEVINFO_REP_FILTER_NET, pci_addr, &pci_device);
	//DOCA_LOG_INFO("return open_doca_device_rep_with_pci: %d",result);
	if (result != DOCA_SUCCESS)
		goto err;
	...
	DOCA_LOG_INFO("Creating a new system handler success");

	/* Set the system os type - linux/widows */
	result = doca_apsh_sys_os_type_set(sys, os_type);
	if (result != DOCA_SUCCESS)
		goto err;	
	DOCA_LOG_INFO("Setting the os type success");
