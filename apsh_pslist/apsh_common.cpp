...
static doca_error_t vuid_callback(void *param, void *config)
{
	...
	if (strnlen(static_cast<const char*>(param), size) >= size) {
		DOCA_LOG_ERR("System VUID argument too long. Must be <=%zu long", size - 1);
		return DOCA_ERROR_INVALID_VALUE;
	}
	strcpy(conf->system_vuid, static_cast<const char*>(param));
	return DOCA_SUCCESS;
}
...
static doca_error_t dma_callback(void *param, void *config)
{
	...
	strcpy(conf->dma_dev_name, static_cast<const char*>(param));
	return DOCA_SUCCESS;
}
