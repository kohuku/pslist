			      ...
			      struct doca_dev_rep **retval)
{
	...
	*retval = NULL;
    ...
    // else {
    //     for (i = 0; i < nb_rdevs; i++){
    //         DOCA_LOG_ERR("device name : %s",rep_dev_list[i]);
    //     }
    // }
    DOCA_LOG_ERR("device count : %d",nb_rdevs);
    DOCA_LOG_ERR("designated device : %s", pci_addr);

	for (i = 0; i < nb_rdevs; i++) {
		result = doca_devinfo_rep_is_equal_pci_addr(rep_dev_list[i], pci_addr, &is_addr_equal);
		if (result == DOCA_SUCCESS && is_addr_equal &&
		    doca_dev_rep_open(rep_dev_list[i], retval) == DOCA_SUCCESS) {
            DOCA_LOG_ERR("Matching device found");
			doca_devinfo_rep_destroy_list(rep_dev_list);
            DOCA_LOG_ERR("destroy rep_dev_list success");
            DOCA_LOG_ERR("DOCA_SUCCESS number is %d", DOCA_SUCCESS);
